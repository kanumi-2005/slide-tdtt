...
\begin{frame}
	...
	\begin{itemize}
		\item Hai đường đi đơn bất kì từ nút gốc đến nút lá có số nút chênh nhau không quá 2 lần.
...
\begin{frame}
	...
	\begin{twotrees}[pause]
	{
		\begin{forest}
				...
				[C
					[A,red-node
						...
						[B,red-node,label=right:x
							[$\beta$,non-decorate-node]
							[$\gamma$,non-decorate-node,name=gamma]
						]
					]
					[$\delta$,non-decorate-node,label=right:u,name=delta]
				]
			...
			\draw[->, bend right=20] (delta.south) to (B.west);
			\pause
			\node (B) at ([xshift=0.5cm,yshift=-0.5cm] gamma) {đen}; % node ở cuối mũi tên
			\draw[->, bend right=20] (gamma.south) to (B.west);
		\end{forest}
	}
	{
		\begin{forest}
			...
			[,empty-node
				[C
					...
					[$\delta$,non-decorate-node,label=right:u,label=delta]
				]
			]
		\end{forest}
	}
