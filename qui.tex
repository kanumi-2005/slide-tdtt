...
\newcommand{\VeNuaTamGiac}[5][right]{
    ...
    \ifthenelse{\equal{#1}{left}}{
        ...
        \node[anchor=east] at ($(Dinh) + (-#4/2, -#3/2)$) {\tiny #5};
    }{
        % =======================
        ...
        \node[anchor=west] at ($(Dinh) + (#4/2, -#3/2)$) {\tiny #5};
    }
}
...
\begin{frame}
    ...
    \begin{description}
        ...
        \item[balance] Hệ số cân bằng của nút.
    \end{description}
\end{frame}


\begin{frame}
    \frametitle{Định nghĩa chiều cao}
    Chiều cao của một nút là độ dài đường đi dài nhất từ nút đó đến lá
    \[
    h(x) = \begin{cases} 
        0 & \text{nếu $x = \mathrm{nil}$} \\
        1 + \max(h(x.left), h(x.right)) & \text{nếu $x \neq \mathrm{nil}$}
    \end{cases}
    \]
\end{frame}
...
\begin{frame}
    ...
    Cây AVL được đặt tên theo Adelson-Velsky và Landis, là cây nhị phân tìm kiếm thỏa mãn chiều cao hai cây con của mỗi nút sai khác nhau không quá một
\end{frame}
...
\begin{frame}
    ...
    \begin{forest}
        for tree={s sep= 3mm},
        tree
        [35,label=above right:{\scriptsize -1}
            [20,label=above left:{\scriptsize 0}
                ...
                [25,label=above left:{\scriptsize 1}
                    [,nil-node]
                    [30,label=above right:{\scriptsize 0}
                        [,nil-node]
                        [,nil-node]
                    ]
                ]
...
\begin{frame}
    ...
    }
    {
        ...
        \begin{forest}
            ...
            for tree={l sep=0pt, s sep=0.5em}
            [,empty-node
                [C,s sep=1em,label=above right:{\scriptsize 0}
...
\begin{frame}
    ...
    }
    {
        ...
        \begin{forest}
            ...
            for tree={l sep=0pt, s sep=0.5em}
            [,empty-node
                [B,s sep=1em,label=above right:{\scriptsize 0}
...
\begin{frame}
    \frametitle{Phép chèn trong cây AVL}
    \begin{block}{Thuật toán chèn nút cây AVL}
    \begin{enumerate}
    \item Thực hiện phép chèn của BST
    \item Cập nhật hệ số cân bằng của nút hiện tại 
    \item Thực hiện thao tác cân bằng nếu nút mất cân bằng
    \item Lặp lại bước 2--3 cho đến nút gốc
    \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}
    \frametitle{Phép xóa trong cây AVL}
    \begin{block}{Thuật toán xóa nút cây AVL}
    \begin{enumerate}
    \item Thực hiện phép xóa của BST
    \item Cập nhật hệ số cân bằng của nút hiện tại 
    \item Thực hiện thao tác cân bằng nếu nút mất cân bằng
    \item Lặp lại bước 2--3 cho đến nút gốc
    \end{enumerate}
...
\begin{frame}
    \frametitle{Đánh giá thao tác cân bằng}
    Độ phức tạp thao tác cân bằng trong phép chèn và xóa trong cây AVL là $\Theta(\log n)$
\end{frame}